...
一部フォント/図版ブロックで通常の read 経路では表示に必要なデータが取れない場合に限り、
純正ドライバ風の op34 fallback を試します。既定で有効です。
問題切り分け時は `USBPC_DISABLE_OP34=1` で無効化できます。
必要なら `USBPC_OP34_SESSION=79` のように tail の session byte を 16 進指定できます。
...
までご連絡ください。内容を確認のうえ、必要に応じて修正・削除等の
対応を行います。
