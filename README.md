...
- **一部の図版が表示されない**ことがあります。図グリフを収めるフォントファイル
  （例 `S3StdFnt.bin`）の**論理EOF超のブロック**を、我々の読み経路（op37 バイトストリーム読み）は
  ゼロ埋めして返すためです。純正ドライバは物理セクタ読み（op34）で同じブロックの実データを取得しますが、
  op34 はデバイス側セッションが確立する内部ハンドルを要し、**我々の WinUSB セッションからは
  同じ物理ブロックがゼロしか返らない**ことを実機採取＋総当たりで確認済み（キャッシュ ON/OFF・
  デバイス再接続とも無関係）。検索・本文・多くの図は正常です。修正にはデバイスのセッション/
  ファイルロード機構のさらに深い解析が必要で、現状は既知の制限としています。

  Some figures may not render. The glyph/font files (e.g. `S3StdFnt.bin`) hold figure
  glyphs in blocks past the file's logical EOF; our read path (op37 byte-stream read)
  zero-fills there. The genuine driver reads the same blocks via a physical-sector read
  (op34), but op34 requires a device-side session handle that our WinUSB session cannot
  reproduce - captured ground truth plus a full brute-force of the session byte confirm
  the device returns only zeros for those blocks in our session (independent of the
  cache or reconnecting the device). Search, entry text, and most figures work. A fix
  would require deeper reverse engineering of the device's session/file-load mechanism.
