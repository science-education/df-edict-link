...
同じ索引ブロックが繰り返し読まれるため、固定サイズ（4096B）ブロックを
LRU でキャッシュします。現在の実装では、キャッシュキーを
`(ファイルパスの 64bit FNV-1a ハッシュ, offset)` としています。

これは意図的にリモートハンドルを使っていません。デバイスが返す
リモートハンドル番号は別ファイルで再利用されることがあり、
`(remote_handle, offset)` をキーにすると、close / 再オープン /
新規 open の隙で別ファイルの古いブロックを返す危険があります。
一方、index 9 の対象は読み取り専用の辞書/索引ファイルなので、
`(ファイルパス, offset)` はセッション中ずっと不変です。このため、
ハンドル再利用による取り違えが原理的に起きず、個別の無効化ロジックも
不要です。

キャッシュは既定で有効です。調査時は `USBPC_NOCACHE=1` で無効化できます。

### op34 font fallback

純正ドライバは index 9 を wire 上では `(handle << 16) | 0x0034`
の物理/ランダムブロック読みとして送ります。tail は観察上
`[a2 >> 8, a2 & 0xff, a5 & 0xff, session, ptr32]` で、`session` は
純正ドライバ内部で確立された値（観察例では `0x79`）です。

通常の index 9 は、seek + read（op37）で同じファイルオフセットを読みます。
これは検索・通常の索引読みには十分ですが、一部の図版/図グリフ用データでは
純正 op34 だけが表示に必要な実データを返すケースがあります。

2026-07-07 の追加調査で、純正 wire の op34 上位 16bit は PASORAMA に
返した DLL ハンドル値と一致し、op37 open の ACK 値とは別のハンドル空間で
あることを確認しました。また `tail[0]` は常に 0 ではなく `a2 >> 8`
です。従来、op37 ACK 値（remote handle）で op34 を forge していたため
全ゼロでしたが、本 DLL が返した local handle と正しい `tail[0]` を使うと、
既知の WPD.bin 高位ブロック（例 `a2=0x167`）と S3StdFnt.bin の該当ブロックは
4096B の非ゼロデータを返しました。

現在はフォント系ファイルで index 9 の op37 読みが短い/4096B 全ゼロだった場合、
または WPD.bin の高位ブロック（`a2 >= 0x100`）の場合に限り、
local handle の op34 fallback を試します。非ゼロデータが返らない場合のみ
従来の remote handle も試します。既定で有効です。
問題切り分け時は `USBPC_DISABLE_OP34=1` で無効化できます。
`USBPC_OP34_SESSION` で tail[3] の session byte を 16 進指定できます
（未指定時は観察値 `79`）。
